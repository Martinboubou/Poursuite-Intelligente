...
void setup() {
  ...
  //size(camWidth, camHeight,OPENGL);                  // some users have reported a faster framerate when the code utilizes OpenGL. To try this, comment out this line and uncomment the line below.
size(640, 480);
  minim = new Minim(this);
 // loadSounds();
 // playSound(18);
  camInput = new IPCapture(this, "http://192.168.137.2:80/video.cgi","admin","poursuite");
  camInput.start();
  camInput.read();
...
void manualMode() {
//  cursor(1);
 camInput.read(); // Remplacement de caminput.update par caminput.updatepixels
 rawBackground = camInput.pixels;
 rawImage = camInput.pixels;
  if (mirrorCam) {
    for (int i = 0; i < camWidth*camHeight; i++) {
